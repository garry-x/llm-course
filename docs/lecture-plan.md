...
- FSDP2 / DTensor 的核心抽象：分片参数不是普通 tensor 副本，checkpoint/resume 和 mixed precision 需要和 shard metadata 对齐。
...
## Week 7 Lecture 14: Preference Optimization、DPO、GRPO 与 Reasoning RL
...
- 解释 DAPO/GSPO 这类 reasoning RL recipe 为什么要关注 rollout 分布、长度增长、entropy、clip fraction、sequence ratio 和 MoE 路由稳定性。
...
- DAPO/GSPO 日志：dynamic sampling 命中率、completion length p50/p95、entropy、clip fraction、approx KL、sequence-level ratio 和 held-out prompt 切片必须和 reward 曲线一起看。
...
- 比较 token-level ratio 与 sequence-level ratio，说明长度归一化和 MoE routing volatility 会改变 RL 日志解释。
...
- DAPO/GSPO 的平均 reward 提升为什么不能单独证明 reasoning 能力变强？
- 为什么“答案可验证”不等于“grader 一定可靠”？

课后产出：

- A7 post-training data audit、RM/DPO/PPO/GRPO/RLVR grader 测试通过，并能解释 DAPO/GSPO 日志中的长度、entropy、clip 和 sequence ratio。
- 阅读复盘：R1/GRPO、DAPO、GSPO、Kimi k1.5 或 RFT 中的 reward 设计与训练稳定性边界。
...
- 给定强/弱模型、router score、成本、SLO 和质量切片，判断 static routing、semantic routing、cascade escalation 或 reliability fallback 是否可上线。
...
- 为什么模型路由不能只报告平均成本下降，而必须按任务/安全/schema/RAG/长上下文切片检查弱模型误路由？
...
- Ch10 KV cache、benchmark summary、overload response、P/D pool plan、observability trace、speculative serving gate 和 long-context serving gate 测试通过。
- 推理项目提案，若 workload 有长 prompt/RAG/多模态/agent 请求，附 prefill、KV transfer、decode queue、TPOT、active KV tokens、trace span 和 overload runbook 的测量计划。
...
- 把 OpenTelemetry GenAI / serving engine metrics 映射到 request、engine、trace、quality/cost 四类遥测。
- 区分 static routing、semantic/classifier routing、cascade escalation 和 reliability fallback，并说明每种模式需要哪些 route receipt。
...
- Observability：request metrics、engine metrics、trace spans 和 content/quality events 分别服务 SLO、容量、单请求归因和质量/安全抽样；prompt/completion/tool observation 默认需要脱敏、采样和访问控制。
- Model routing gate：route_reason、router score、branch sample size、quality/safety/schema/RAG regression、P95 latency、cost/request、fallback loop 和人工抽检必须与强模型 baseline 对照。
...
- 给定一组 route receipts，计算弱模型命中率、升级率、fallback error rate 和成本节省，判断是否因为某个高风险切片误路由而 block release。
...
- 为什么 fallback 到另一个模型时仍要检查 tokenizer、chat template、tool schema、response_format 和安全策略兼容性？
