...
- `post_training_data_audit` 必须把 SFT、preference 和 RLVR/RFT 数据源审计成 coverage、label quality、leakage 和 safety gate，报告任务/安全切片覆盖、长度偏差、同 prompt 标签冲突、eval overlap、unsafe chosen rate 和是否可以进入 post-training。
