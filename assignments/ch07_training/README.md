...
本作业对应第 7 章训练循环。目标是把 next-token 数据切片、数据重复/泄漏诊断、稳定交叉熵、AdamW、warmup+cosine 调度和一个可复现的小训练循环串起来。
...
| 项目 | 分值 | 标准 |
|------|:--:|------|
| Written questions | 35 | 推导交叉熵、CE 对 logits 的梯度、perplexity、AdamW 偏置修正、warmup+cosine 边界、n-gram 泄漏诊断和 grad clipping 的诊断意义 |
| Programming parts | 55 | 实现 dataset/dataloader、n-gram 重复/重叠率、稳定 cross entropy、AdamW、scheduler 和训练循环 |
