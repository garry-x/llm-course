...
本目录把第 4 章的编程练习整理成可自动验收的作业入口，覆盖 MHA、单头参数等价性、GQA head mapping、简化版 MLA 和 KV Cache 大小计算。
...
| `tests.py` | `unittest` 测试，覆盖 shape、mask、参数量、KV 头重复、GQA head mapping 和 cache 压缩比 |
...
| 项目 | 分值 | 标准 |
|------|:--:|------|
| Written questions | 35 | 计算 MHA 参数量，比较 MHA/MQA/GQA/MLA 的 KV cache，写出 Q head 到 KV head 的映射，解释 head redundancy、RoPE 与 latent cache 的边界 |
| Programming parts | 55 | 实现 MHA、单头对照、GQA、GQA head mapping、简化 MLA 和 KV cache 分析 |
