...
| Evaluation | intrinsic/extrinsic、accuracy/F1/BLEU/ROUGE/perplexity、LLM judge 风险、bootstrap confidence interval、seed sensitivity | 补充作业：实现 BLEU、ROUGE-L、QA EM/F1；项目报告：至少一个自动指标、一个人工错误分析，并说明数据划分、指标局限和不确定性 |
...
- 说明 train/test duplicate、prompt leakage、retrieval contamination 或 benchmark contamination 如何处理。
