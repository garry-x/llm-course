...
2. 给定 tokenized messages：`system` 2 tokens、`user` 3 tokens、`assistant` 4 tokens、`user` 2 tokens、`assistant` 3 tokens，其中第二个 assistant `weight=0`；再给 `max_seq_len=10`、`pack_examples=True`、`block_diagonal_attention=False`。标出 assistant spans、supervised tokens、supervised_token_ratio、被截断的 assistant tokens，以及 `sft_chat_template_mask_report` 应失败的 gate 和 action items。
3. 从 Bradley-Terry 偏好模型写出 DPO loss 中 chosen/rejected log-ratio 的含义。
4. 给定 policy chosen/rejected log-probs `[3,1]`、`[1,2]`，reference chosen/rejected log-probs `[1,1]`、`[1,1]`，`beta=0.5`，计算 DPO 隐式 chosen/rejected reward、reward margin、preference probability 和 preference accuracy。
5. 给定一组 chosen/rejected reward，计算 Bradley-Terry pairwise reward loss 和 preference accuracy，并解释这个 loss 与奖励模型训练的关系。
6. 给定一组 chosen/rejected 回答长度，计算 mean length delta、chosen_longer_rate、rejected_longer_rate 和 tie_rate，并说明长度偏差如何影响 DPO 或 RLHF。
7. 给定一张 SFT/preference/RLVR 样本表，手动计算 task coverage、safety slice coverage、max task share、label conflict rate、mean length delta ratio、eval overlap rate 和 unsafe chosen rate，并判断 `post_training_data_audit` 的 coverage、label_quality、leakage、safety 哪些 gate 失败。
8. 给定一组 chosen/rejected 回答，指出可能的风格偏差或标注者分歧，并说明这些偏差如何影响 DPO 或 RLHF。
9. 给定 policy/ref 在 sampled tokens 上的 log probability，计算近似 KL `exp(log_ref-log_policy) - (log_ref-log_policy) - 1`，并说明 padding mask 为什么不能进入均值分母。
10. 给定 old/new policy log-probs、advantages 和 `clip_range=0.2`，计算 PPO unclipped surrogate、clipped surrogate、最终 policy loss、clip fraction 和 approximate KL；说明 PPO clipping 与 KL penalty 分别约束什么。
11. 给定 GRPO rewards `[[1],[2],[3]]`、old log-probs 全 0、new/old ratios `[[[1.0,1.5]],[[1.0,1.0]],[[0.5,1.0]]]`、completion mask `[[[1,1]],[[1,0]],[[1,1]]]`、reference log-probs 全 0、`clip_range=0.2`、`kl_beta=0.04`，计算组内白化 advantages、PPO clipped surrogate、masked policy loss、masked approximate KL 和总 loss；说明 GRPO 组内白化不能解决哪些 reward hacking 问题。
12. 比较 LoRA rank、alpha、target modules 对训练参数量、表达能力和合并推理的影响。
13. 设计一个评估 DPO 模型是否优于 SFT/reference 的方案：至少包含 helpfulness、事实性、安全拒答、过度拒答和数学/代码能力保留，并解释为什么 preference win rate 不能单独作为结论。
...
7. 给定结构化输出记录：一条 invalid JSON，一条 parse 通过但 enum 不在 schema 中，一条有 extra field 且 retry 后 P95 延迟超阈值；policy 要求 JSON/schema valid rate `>=0.9`、retry rate `<=0.2`、fallback/safety violation rate 为 0。计算 `structured_output_reliability_report` 的 JSON parse、schema adherence、retry/latency、fallback/refusal、safety gate，并说明为什么 JSON 可解析不等于可以上线。
8. 给定一个 MCP/remote tool 事件：tool server 不在 allowlist、risk=`write`、permissions=`read/write`、未获得用户批准、参数中包含敏感数据、工具返回外部网页且包含 prompt injection、observation 未隔离、server 请求 recursive LLM sampling、总调用次数和 observation tokens 超过 policy。分别判断 server trust、permission/consent、data privacy、observation isolation、sampling/budget 哪些 gate 失败，并给出 action items；说明为什么 JSON schema 通过不代表工具可执行。
9. 给定 stable baseline：`pass_rate=0.88`、`safety_pass_rate=0.97`、`cost_per_1k_requests_usd=1.20`；候选发布包：`pass_rate=0.90`、`safety_pass_rate=0.965`、`p95_latency_ms=860`、`error_rate=0.008`、`cost_per_1k_requests_usd=1.32`、`canary_traffic_pct=5`、`canary_sample_size=1200`、已与 control 对比、已准备 rollback、监控字段包含 pass/safety/P95/error/cost。在 policy `min_pass_rate=0.86`、`max_quality_regression_abs=0.02`、`min_safety_pass_rate=0.95`、`max_safety_regression_abs=0.02`、`max_p95_latency_ms=900`、`max_error_rate=0.01`、`max_cost_regression_pct=0.2`、`min_canary_sample_size=1000`、`max_canary_traffic_pct=10` 下，逐项判断 offline quality、safety、SLO、cost、canary、rollback/monitoring gate，并给出 promote、继续低流量 canary 还是 rollback/block。再说明为什么离线 eval 提升不能跳过 canary、control、per-version monitoring 和 rollback readiness。
10. 给定运行期指标：`waiting_requests=80`、`p95_queue_wait_ms=900`、`p95_ttft_ms=1800`、`p95_tpot_ms=95`、`kv_cache_usage_pct=0.96`、`swapped_requests=3`、`error_rate=0.03`、`timeout_rate=0.02`、`degradation_ready=False`；policy：`max_waiting_requests=20`、`max_p95_queue_wait_ms=200`、`max_p95_ttft_ms=900`、`max_p95_tpot_ms=60`、`max_kv_cache_usage_pct=0.9`、`max_swapped_requests=0`、`max_error_rate=0.01`、`max_timeout_rate=0.005`；租户 `free-tier` 的 request rate 是 quota 的 4 倍。逐项判断 queue pressure、KV pressure、decode saturation、error budget、tenant fairness 和 degradation readiness gate，给出 severity、decision 和 action items；说明为什么负载削减、降级、扩容、回滚和 noisy-neighbor 隔离不能混成同一个“服务慢了”结论。
11. 比较 INT8 weight-only quantization、KV Cache quantization 和 FP8/FP4 mixed precision 的目标、风险和验证指标。
12. 说明为什么前沿模型 benchmark 数字必须同时给出任务、数据集、推理设置和评测版本。
13. 设计一个 RAG 消融实验：比较 dense-only、BM25-only、hybrid、hybrid+rerank、hybrid+MMR 和两组 chunk size/overlap，说明应分别报告哪些检索指标、生成质量指标、延迟和 token 成本。
14. 设计一个多模态 LLM 评估：分别覆盖图像问答、OCR/文档理解、图表数值推理和视觉定位，说明输入分辨率、视觉 token 数、延迟、KV Cache 成本和每类任务的失败模式。
15. 为一个 LLM benchmark 写结构化结论摘要：包括 task、sample_size、baseline、metrics、risks、uncertainty 和 conclusion，并说明哪些结论不能从这组数据推出。
15. 给定服务 workload：`QPS=10`、平均 prompt tokens `800`、平均 output tokens `200`、单卡 prefill capacity `120k prompt tokens/s`、decode capacity `2.5k output tokens/s`，计算 prefill/decode token rate，并判断主要瓶颈；说明为什么 QPS 继续上升时 TPOT、排队时长和 TTFT 可能以不同顺序恶化。
16. 给定 32 层 GQA 模型、`n_kv_heads=8`、`head_dim=128`、fp16 KV Cache、平均活跃上下文 `6000` tokens、可用于 KV 的显存 `48 GiB`，计算每 token KV bytes、每请求 KV Cache 和理论最大活跃请求数；再考虑 20% 安全余量，给出 admission limit，并解释为什么只按并发请求数限流会误伤短请求或放过长请求。
17. 给定 speculative decoding 服务 trace：baseline `2000 ms`、speculative `940 ms`、输出 `200` tokens、draft `200` tokens、accepted `152` tokens、target verify steps `50`、draft cost `200 ms`、quality regression `0`、额外显存 `1.5 GiB`、QPS `5`。在 gate `acceptance>=0.7`、`speedup>=1.8`、`draft_overhead<=0.25`、额外显存 `<=2 GiB` 下，计算 acceptance rate、speedup、draft overhead、tokens per verify step，并判断是否启用；再说明为什么 high QPS、compute-bound workload 或质量回归会改变结论。
18. 给定长上下文服务记录：每条包含 `context_tokens`、`max_context_tokens`、`reserved_output_tokens`、`needle_position_pct`、`answer_correct`、`citation_hit`、`ttft_ms`、`latency_ms`、`kv_cache_usage_pct` 和 `prefix_cache_hit_rate`。按 `long_context_serving_gate_report` 计算 truncation rate、answer/citation recall、head/middle/tail bucket accuracy、P95 TTFT、P95 latency、最大 KV 使用率和平均 prefix cache hit rate；判断 context fit、long-context quality、position robustness、serving cost gate，并说明为什么“支持 1M tokens”不能替代 needle/citation/position/SLO 回归。
