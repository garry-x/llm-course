...
这份文档把课程从“学完 10 章”转换为“达到 LLM 推理工程师入门岗位能力”。它不是额外章节，而是学习、复盘和验收用的路线图。
...
| 5. 质量与上线 | Ch09-Ch10、Capstone | 如何评测输出质量、安全、格式和成本 | 有固定评测集、上线清单和压测报告 |
...
- 能设计检索命中文档、prompt 注入、引用返回和失败兜底。
- 能说明 JSON schema / constrained decoding 比“请输出 JSON”的 prompt 更可靠。

**对应内容：**Ch08 约束生成，Ch10 10.8、10.13，Capstone RAG stub。
