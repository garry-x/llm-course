...
本作业对应第 8 章文本生成。目标是实现从 logits 到 token 的核心解码策略，包括概率截断、重复惩罚、搜索和多样性指标，并用小模型验证采样边界和生成行为。
...
| Programming parts | 55 | 实现 greedy/beam/temperature、top-k、top-p、repetition penalty、pass@k、Generator 指标和 speculative decoding |
