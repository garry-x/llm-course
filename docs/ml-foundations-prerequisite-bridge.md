...
本 handout 汇总本课程会反复用到的 college calculus、probability/statistics 和 foundations of machine learning。它不是独立机器学习课程，而是把学生需要带入本课程的最小概念映射到 LLM 章节、作业和书面题。相关复习材料见 [数学与 PyTorch 先修复习](math-prerequisites.md)、[Python and PyTorch Review Session](python-pytorch-review-session.md) 和 [逐周阅读材料 Handout](reading-list.md)。
