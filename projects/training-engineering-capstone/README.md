...
这个项目把第 7 章和第 9 章的训练知识落成一个可验收的训练工程作品：一个 PyTorch 字符级语言模型训练闭环，覆盖数据审计、训练、验证、checkpoint、resume、metrics、训练规划和一键验收。
...
| 模块 | 最低要求 | 验收证据 |
|------|----------|----------|
| Data Audit | 样本数、空样本、重复率、长度分布、字符/token 规模 | `python data_audit.py` 输出 JSON |
...
| Acceptance | 串联数据审计、训练、resume、规划和指标检查 | `python acceptance.py` 输出 `ACCEPTANCE: PASS` |
...
```bash
python data_audit.py --data sample_corpus.txt
```
