...
2. 给定 token 序列 `[0,1,2,1]` 和 window size 1，手算 directed co-occurrence matrix；再给定 center/positive/negative 向量，写出 SGNS loss；给定非零共现计数、word/context 向量和 bias，计算一项 GloVe weighted least-squares residual。
...
4. 给定 attention weights `[1,0,0]` 和 `[1/3,1/3,1/3]`，计算 attention entropy，并说明它如何反映 softmax 分布尖锐程度。
5. 分析 self-attention 的时间复杂度和显存复杂度；给定 `B,H,T,dtype_bytes`，计算 dense attention score 矩阵的显存，并区分训练时保存 activation 与推理时 KV Cache 的差异。
6. 给定一张 attention heatmap，说明它能支持哪些路由诊断、不能单独支持哪些因果解释；设计一个 ablation 或 counterfactual prompt 比较模型行为变化。
...
7. 对数学或代码题设计一个 reasoning 生成实验：比较 single-sample、self-consistency、best-of-N 和 verifier reranking；给定 `n` 个样本中 `c` 个正确，计算 `pass@k = 1 - C(n-c,k)/C(n,k)`，并报告平均输出 token 数、延迟和单位正确答案成本。
...
3. 给定 query/document embedding batch，写出 in-batch negatives 的 InfoNCE logits、label 和 loss；给定 dense 检索排序、BM25 排序、retrieved document ids、relevant document ids、graded relevance scores 和 `k`，计算 RRF 融合排序、Recall@k、reciprocal rank 与 nDCG@k，并说明它们分别衡量 RAG 检索的哪一类问题。
...
4. BERT/Encoder-only：比较 MLM 与 causal LM 的 mask 方式、训练目标和下游 fine-tuning 输入格式；给定 BIO tags 解码 NER/entity spans；给定 emission/transition scores 手算 Viterbi 最优 tag path 和 CRF forward log-partition；给定 `[CLS] question [SEP] passage [SEP]` 的 start/end logits，选择抽取式 QA 的最佳 span，并说明 `[CLS]` no-answer 的含义。
