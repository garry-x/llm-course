...
本目录把第 4 章的编程练习整理成可自动验收的作业入口，覆盖 MHA、单头参数等价性、GQA head mapping、简化版 MLA、KV Cache 大小计算和跨层显存预算。
...
| `tests.py` | `unittest` 测试，覆盖 shape、mask、参数量、KV 头重复、GQA head mapping、cache 压缩比和 batch/layer/dtype 显存预算 |
...
| Programming parts | 55 | 实现 MHA、单头对照、GQA、GQA head mapping、简化 MLA、KV cache 分析和跨层显存预算 |
