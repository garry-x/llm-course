...
4. BERT/Encoder-only：比较 MLM 与 causal LM 的 mask 方式、训练目标和下游 fine-tuning 输入格式；给定 BIO tags 解码 NER/entity spans；给定 emission/transition scores 手算 Viterbi 最优 tag path、CRF forward log-partition、gold path score 与 CRF NLL；给定 `[CLS] question [SEP] passage [SEP]` 的 start/end logits，选择抽取式 QA 的最佳 span，并说明 `[CLS]` no-answer 的含义。
